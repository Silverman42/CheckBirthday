...
<template>
  <div class="grid grid-cols-1 gap-2 bg-white">
    ...
    <ul class="grid grid-cols-1 border border-[#F4F4FD] rounded-xl max-h-[11rem] overflow-y-auto">
      <li
        ...
        :class="{
          'text-primary bg-primary/5': selectedValue?.id === data.id,
          'text-primary-dark': selectedValue?.id !== data.id,
        }"
      >
        <slot name="listData">
          {{ data.name }}
